...
//  }
    ...
    function getLavaPacketTypehash() public view returns (bytes32) {
...
/*  function getDomainHash(EIP712Domain eip712Domain)  pure returns (bytes32) {
    ...
    function getLavaPacketHash(LavaPacket memory packet) public view returns (bytes32) {
        return keccak256(abi.encode(
            LAVAPACKET_TYPEHASH,
  ...
  function balanceOf(address token,address user) public view returns (uint) {
   ...
   function allowance(address token, address tokenOwner, address spender) public view returns (uint remaining) {
  ...
  // function getLavaPacket(address from,)
   ...
   function getLavaTypedDataHash(LavaPacket memory packet) public view returns (bytes32) {
   ...
   //EIP 712
/*   function getLavaTypedDataHash(bytes methodName, LavaPacket packet ) public view returns (bytes32)
   {
  ...
  //the tokens remain in lava wallet
 function withdrawTokensWithSignature(LavaPacket memory packet, bytes memory signature) public returns (bool success)
 {
