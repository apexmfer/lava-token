...
contract StakingDelegate is  RelayAuthorityInterface {
  ...
  function startStaking(uint tokens , uint stakerIndex, address authority) public returns (bool)
  {
  ...
  function stopStaking(uint stakerIndex) public returns (bool)
  {
    ...
    stakers[stakerIndex] = address(0x0);
    stakerAuthorities[stakerIndex] = address(0x0);
  ...
  */
  ...
  function getEpochNumber() view public returns (uint256  )
  {
...
function depositTokens(address from,  uint256 tokens ) public returns (bool success)
  {
      ...
      require(ERC20Interface(minedToken).transferFrom(from, address(this), tokens));
